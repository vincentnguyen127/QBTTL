...
Public Class IntegratedUI
    ...
    Private Sub handleTag(sender As Object, e As EventArgs) Handles AttributeTabControl.SelectedIndexChanged
        ...
        If Type = 20 Then
            ...
            RetunValue = UIRelatedFunctions.display_TimeEntry_UI()
    ...
    '    My.Forms.IntegratedUI.ProgressBar1.Value = 0
    ...
    Private Function display_UI() Handles QBtoTLCustomerRadioButton.CheckedChanged, QBtoTLEmployeeRadioButton.CheckedChanged,
        ...
        My.Forms.IntegratedUI.TimeEntrySelectAll.Visible = False
        ' Unselect the select all check box
        My.Forms.IntegratedUI.SelectAllCheckBox.Checked = False

        Select Case Type
            ' Customers
            Case 10
                My.Forms.IntegratedUI.TabPageCustomers.Visible = True
                My.Forms.IntegratedUI.AttributeTabControl.SelectedIndex = 0
                lastSync = My.Settings.CustomerLastSync
                customerData = customer_qbtotl.GetCustomerQBData(Me, True)
                Data = customerData
                attribute = "customer"
                QBtoTLRadioButton = My.Forms.IntegratedUI.QBtoTLCustomerRadioButton

            ' Employees
            Case 11
                My.Forms.IntegratedUI.TabPageEmployees.Visible = True
                My.Forms.IntegratedUI.CustomerSyncDirection.Visible = True
                My.Forms.IntegratedUI.AttributeTabControl.SelectedIndex = 1
                lastSync = My.Settings.EmployeeLastSync
                employeeData = employee_qbtotl.GetEmployeeQBData(Me, True)
                Data = employeeData
                attribute = "employee"
                QBtoTLRadioButton = My.Forms.IntegratedUI.QBtoTLEmployeeRadioButton

            ' Vendors
            Case 12
                My.Forms.IntegratedUI.TabPageVendor.Visible = True
                My.Forms.IntegratedUI.AttributeTabControl.SelectedIndex = 2
                lastSync = My.Settings.VendorLastSync
                vendorData = vendor_qbtotl.GetVendorQBData(Me, True)
                Data = vendorData
                attribute = "vendor"
                QBtoTLRadioButton = My.Forms.IntegratedUI.QBtoTLVendorRadioButton

            ' Jobs / Subjobs
            Case 13
                My.Forms.IntegratedUI.TabPageJobsItems.Visible = True
                My.Forms.IntegratedUI.AttributeTabControl.SelectedIndex = 3
                lastSync = My.Settings.JobLastSync
                JobData = job_qbtotl.GetJobSubJobData(Me, p_token, True)
                Data = JobData
                attribute = "job/subjob"
                QBtoTLRadioButton = My.Forms.IntegratedUI.QBtoTLJobItemRadioButton
        ...
        My.Forms.IntegratedUI.SyncFromLabel.Text = If(Type = 20, "Employees", If(QBtoTLRadioButton.Checked, "QuickBooks", "TimeLive"))
        My.Forms.IntegratedUI.SyncToLabel.Text = If(Type = 20, "Time", If(QBtoTLRadioButton.Checked, "TimeLive", "QuickBooks"))
        ...
        End If
        My.Forms.IntegratedUI.History("Synchonizing modified " + attribute + " since: " + ItemLastSync.ToString(), "n")

        ' Delete all rows and columns from the DataGridView's except the "Check Name" column in  My.Forms.IntegratedUI.DataGridView1
        Try
            My.Forms.IntegratedUI.DataGridView1.Rows.Clear()
        Catch nullEx As NullReferenceException
            ' Do Nothing - just don't throw the exception
        End Try
        While My.Forms.IntegratedUI.DataGridView1.ColumnCount > 1
            Try
                My.Forms.IntegratedUI.DataGridView1.Columns.RemoveAt(1)
            Catch nullEx As NullReferenceException
                ' Do nothing - just don't throw the exception
            End Try
        End While
        ...
        Try
            My.Forms.IntegratedUI.DataGridView2.Rows.Clear()
            My.Forms.IntegratedUI.DataGridView2.Columns.Clear()
        Catch nullEx As NullReferenceException
            My.Forms.IntegratedUI.DataGridView2 = New DataGridView()
        End Try
        ...
        Dim QBDataGridView As DataGridView = If(QBtoTLRadioButton.Checked, My.Forms.IntegratedUI.DataGridView1, My.Forms.IntegratedUI.DataGridView2)
        Dim TLDataGridView As DataGridView = If(QBtoTLRadioButton.Checked, My.Forms.IntegratedUI.DataGridView2, My.Forms.IntegratedUI.DataGridView1)
        ...
        If Data Is Nothing Then
            My.Forms.IntegratedUI.History("No QuickBooks " + attribute + " data", "n")
        Else
        ...
        If My.Forms.IntegratedUI.ProgressBar1.Maximum = Nothing Then
            My.Forms.IntegratedUI.ProgressBar1.Maximum = 0
        End If

        My.Forms.IntegratedUI.ProgressBar1.Maximum += If(TLItemsArray Is Nothing, 0, TLItemsArray.Length)

        If TLItemsArray Is Nothing Or TLItemsArray.Length = 0 Then
            My.Forms.IntegratedUI.History("No TimeLive " + attribute + " data", "n")
        End If
        ...
        For Each element In TLItemsArray
                ...
                Case 13
                    ...
                    Else
                        ...
                        Catch ex As System.Web.Services.Protocols.SoapException
                            My.Forms.IntegratedUI.History("Could not get TL ID of TL task '" + name +
            ...
            My.Forms.IntegratedUI.ProgressBar1.Value += 1
        Next
    ...
    Public Sub IntegratedUI_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If Type = 20 Then
            ...
            ReadItems = UIRelatedFunctions.display_TimeEntry_UI()
        End If
    ...
    Public Overloads Sub Show(ByVal token As String, ByVal AccountId As String, TypeSelected As Integer)
        ...
        My.Forms.IntegratedUI.History("Welcome to QB2TL Sync by Telrium", "n")
        Using newForm = New Login()
            If DialogResult.OK = newForm.ShowDialog() Then
                ...
                My.Forms.IntegratedUI.History("You are logged into TimeLive", "n")
                My.Forms.IntegratedUI.History("", "n")
            Else
                My.Forms.IntegratedUI.History("You will need to log into TimeLive before using this utility.", "n")
                My.Forms.IntegratedUI.History("", "n")
            End If
        End Using
    ...
    Private Sub btnRefreshTimeTransfer_Click(sender As Object, e As EventArgs) Handles RefreshTimeTransfer.Click
        ...
        UIRelatedFunctions.Reset_Checked_SelectedEmployee_Value(selectedEmployeeData)
        UIRelatedFunctions.Set_Selected_SelectedEmployee()
        'Dim IntUI_2ndSelect As New IntUI_2ndSelect
        UIRelatedFunctions.Time_Entry_Times()
        ...
        My.Forms.IntegratedUI.ProgressBar1.Maximum = selectedEmployeeData.NoItems
        For Each element As TLtoQB_TimeEntry.Employee In selectedEmployeeData.DataArray
            With element
                If element.RecSelect = True Then
                    History("Processing: " + element.FullName.ToString(), "n")
                    UIRelatedFunctions.LoadSelectedTimeEntryItems(element.AccountEmployeeId, element.FullName, DataGridView2, StartDate, endDate, True)
    ...
    Private Sub btnTransfer_Click(sender As Object, e As EventArgs) Handles btnTransfer.Click
        ...
        If Type = 10 Then
            UIRelatedFunctions.Reset_Checked_Customer_Value(customerData)
            If QBtoTLCustomerRadioButton.Checked Then
                UIRelatedFunctions.QB_Set_Selected_Customer()
                ...
                My.Settings.Save()
            Else
                Dim customersToCheck As List(Of String) = UIRelatedFunctions.TL_Set_Selected_Items()
        ...
        If Type = 11 Then
            UIRelatedFunctions.Reset_Checked_Employee_Value(employeeData)
            If QBtoTLEmployeeRadioButton.Checked Then
                UIRelatedFunctions.Set_Selected_Employee()
                ...
                My.Settings.Save()
            Else
                Dim employeesToCheck As List(Of String) = UIRelatedFunctions.TL_Set_Selected_Items()
        ...
        If Type = 12 Then
            UIRelatedFunctions.Reset_Checked_Vendor_Value(vendorData)
            If QBtoTLVendorRadioButton.Checked Then
                UIRelatedFunctions.Set_Selected_Vendor()
                ...
                My.Settings.Save()
            Else
                Dim vendorsToCheck As List(Of String) = UIRelatedFunctions.TL_Set_Selected_Items()
        ...
        If Type = 13 Or Type = 14 Then
            UIRelatedFunctions.Reset_Checked_Job_Value(JobData)
            If QBtoTLJobItemRadioButton.Checked Then
                UIRelatedFunctions.Set_Selected_Job_Item()
                If Type = 13 Then
                ...
                My.Settings.Save()
            Else
                Dim jobsToCheck As List(Of String) = UIRelatedFunctions.TL_Set_Selected_Items()
        ...
        If Type = 20 Then
            UIRelatedFunctions.Reset_Checked_TimeEntry_Value(TimeEntryData)
            UIRelatedFunctions.Set_Selected_TimeEntry(DataGridView2)
    ...
    '    My.Forms.IntegratedUI.ProgressBar1.Maximum = DataGridView.Rows.Count
    ...
    '                    History(selectedEmployee.FullName + " vs " + row.Cells("Name").Value.ToString, "i")
    ...
    Private Sub btn_currentweek_Click(sender As Object, e As EventArgs) Handles btn_currentweek.Click
        ...
        Dim sat As Date = UIRelatedFunctions.FirstDateOfWeek(Now.Year, cur_week, DayOfWeek.Saturday)
        'sat.AddDays(-1)
    ...
    '    Return UIRelatedFunctions.GetFirstDayOfWeek(year, weekNumber, Application.CurrentCulture)
    'End Function
    ...
    Private Sub nextWeek_Click(sender As Object, e As EventArgs) Handles nextWeek.Click
        cur_week = cur_week + 1
        Dim sat = UIRelatedFunctions.FirstDateOfWeek(Now.Year.ToString, cur_week, DayOfWeek.Saturday)
        If Now.DayOfWeek.CompareTo(sat.DayOfWeek) > 0 Then
            dpStartDate.Value = sat.AddDays(-1)
            dpEndDate.Value = sat.AddDays(6)
